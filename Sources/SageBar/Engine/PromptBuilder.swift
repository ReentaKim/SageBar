...
enum PromptBuilder {
    ...
    static func letterPrompt(persona: Persona, profile: String, recent: String, recentTopics: String,
        ...
        - "내 얘기와 달랐다"가 있으면: 그 글이 상대의 상황을 잘못 읽은 것이다. 인물지의 사실을 다시 확인하고,
          확인되지 않은 것은 단정하지 말고 "헤아리건대"로 추측임을 드러내며, 판단마다 근거 발화를 인용한다.
        ...
        - 소제목 하나는 "관찰한 행동 → 인물의 해석 → 오늘 해볼 한 가지"의 흐름으로 쓰고, 마지막 문장은
          짧고 구체적인 실천 문장으로 맺는다(예: "오늘 저녁, 폴더 스물여섯 개 중 다섯 개만 남기소서").
        - 인물별 문체를 유지하되, 현대 한국어로 읽기 어렵지 않게 한다.
        - 이미 다룬 주제 목록과 겹치지 않는 새 각도를 잡는다.

        ────────── 출력 형식 (반드시 이 형식을 정확히 지킬 것) ──────────
        SUBTITLE: (오늘 글의 핵심 조언을 한 문장으로 — 12~20자, 순한글, 인물 말투의 청유·명령형.
         예: "오늘은 새 도구보다 하나를 끝내소서". 본문에 실제로 근거가 있어야 하며 과장·자극 문구는 쓰지 않는다.
